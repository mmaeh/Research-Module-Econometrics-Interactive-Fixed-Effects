...
for (c in combinations) {
  ...
  for (rep in 1:n_rep) {
    ...
    epsilon <- error_function(t, i, mean = 0, sd = sqrt(2), serial_corr = TRUE, rho = 0.7, serial_hetero = TRUE)
...
saveRDS(results, './output/table_Vd.rds')
write.xlsx(results, './output/table_Vd.xlsx')
