...
for (c in combinations) {
  ...
  for (rep in 1:n_rep) {
    ...
    epsilon <- error_function(T, N, mean = 0, sd = sqrt(2), cross_corr = TRUE, rho = 0.7)
...
# Save results table.
saveRDS(results, './mc_simulations/output/table_cross_wo.rds')
write.xlsx(results, './mc_simulations/output/table_cross_wo.xlsx')
