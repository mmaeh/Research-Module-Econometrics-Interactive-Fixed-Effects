...
for (c in combinations) {
  ...
  for (rep in 1:n_rep) {
    ...
    epsilon <- error_function(T, N, mean = 0, sd = sqrt(2), cross_corr = TRUE, rho = 0.7, cross_hetero = TRUE)
    
    # Create simulated data.
    source('./mc_simulations/data_generating/data_generating_general.R')
...
# Save results table.
saveRDS(results, './mc_simulations/output/table_cross_with.rds')
write.xlsx(results, './mc_simulations/output/table_cross_with.xlsx')
