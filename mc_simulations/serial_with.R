# The following code reproduces Table A.2 of the paper.
...
for (c in combinations) {
  ...
  for (rep in 1:n_rep) {
    ...
    epsilon <- error_function(T, N, mean = 0, sd = sqrt(2), serial_corr = TRUE, rho = 0.7, serial_hetero = TRUE)
...
# Save results table.
saveRDS(results, './mc_simulations/output/table_serial_with.rds')
write.xlsx(results, './mc_simulations/output/table_serial_with.xlsx')
