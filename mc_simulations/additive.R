...
# Y_{it} = X_{it,1} \beta_{1} + X_{it,2} \beta_{2} + \alpha_{i} + \alpha_{t} + \varepsilon_{it}
...
for (c in combinations) {
  ...
  for (rep in 1:n_rep) {
    ...
    source('./mc_simulations/data_generating/data_generating_additive.R')
...
# Save results table.
saveRDS(results, './mc_simulations/output/table_additive.rds')
write.csv(results, './mc_simulations/output/table_additive.csv')
